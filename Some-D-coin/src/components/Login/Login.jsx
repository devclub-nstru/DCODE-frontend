...
function Auth() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      setMessage({ text: "Google sign-in failed. Please try again.", isError: true });
    }
  };
    ...
    <GoogleOAuthProvider clientId="YOUR_GOOGLE_CLIENT_ID">
      <div className="auth-container">
        <div className="auth-box">
          ...
          {isLogin ? <LoginForm handleGoogleSignIn={handleGoogleSignIn} /> : <SignupForm handleGoogleSignIn={handleGoogleSignIn} />}

          <div className="auth-footer">
            {isLogin ? (
              <p>
                Don't have an account? <span className="auth-link" onClick={() => setIsLogin(false)}>Sign up now</span>
              </p>
            ) : (
              <p>
                Already have an account? <span className="auth-link" onClick={() => setIsLogin(true)}>Login here</span>
              </p>
            )}
          </div>
        </div>
